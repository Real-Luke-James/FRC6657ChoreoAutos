...
public class IntakeIO_Sim implements IntakeIO {
  ...
  private DCMotorSim pivotSim =
   new DCMotorSim(
      LinearSystemId.createDCMotorSystem(DCMotor.getNEO(1), 0.0001, Constants.Intake.pivotGearing),
      DCMotor.getNEO(1));
  private DCMotorSim rollerSim = 
    new DCMotorSim(
      LinearSystemId.createDCMotorSystem(DCMotor.getFalcon500(1), 0.0001, Constants.Intake.rollerGearing),
  ...
  public void updateInputs(IntakeIOInputs inputs) {
    ...
    inputs.pivotMotorVelocity =
      pivotSim.getAngularVelocityRPM() * (360d / 60); // Degrees
